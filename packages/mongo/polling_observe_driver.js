PollingObserveDriver = function(options) {
  ...
  Package['facts-base'] &&
    Package['facts-base'].Facts.incrementServerFact('mongo-livedata', 'observe-drivers-polling', 1);
};
...
_.extend(PollingObserveDriver.prototype, {
  ...
  _pollMongo: function() {
    ...
    const cd = self._cursorDescription;
    const name = cd.collectionName;
    const sel = JSON.stringify(cd.selector);
    const opts = JSON.stringify(cd.options);
    console.log(
      `[pollingObserveDriver] ${self.__driverId} Polling mongo - ${name} - ${sel} - ${opts}`
    );
